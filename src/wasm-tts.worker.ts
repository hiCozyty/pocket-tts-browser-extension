...
const postStatus = (status: WasmLoadStatus) => {
...
const fetchWeights = async (
  ...
  if (useCache && cache) {
    return await cache.fetchWithCache(`weights:${hfRepo}`, hfUrl, (loaded, total) => {
      postStatus({
        phase: "loading-assets",
        progress: total ? 30 + Math.floor((loaded / total) * 30) : 40,
        message: total
          ? `Downloading model weights (${(loaded / 1_000_000).toFixed(0)} / ${(total / 1_000_000).toFixed(0)} MB)...`
          : `Downloading model weights (${(loaded / 1_000_000).toFixed(0)} MB)...`,
        source: "hf",
        ready: false,
        error: null,
      });
...
const handleInit = async (message: Extract<WasmWorkerRequest, { kind: "init" }>) => {
  ...
  const weightsBytes = await fetchWeights(
    message.hfRepo,
    message.hfToken,
    cache,
    message.useCache,
  ...
  const configBytes = encoder.encode(DEFAULT_CONFIG_YAML);
  const tokenizerBytes = new Uint8Array(0);

  postStatus({
    phase: "compiling-model",
    progress: 78,
    message: "Compiling model in WASM...",
    source: message.useCache ? "cache" : "hf",
    ready: false,
    error: null,
  });
